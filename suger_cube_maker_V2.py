...
def create_textured_background(size):
    ...
    speckles = np.random.randint(0, 30, (img_size, img_size, 3), dtype=np.uint8)
    
    # Add them together
    # background = cv2.add(background, speckles)
    background = cv2.add(background.astype(np.uint8), speckles.astype(np.uint8))
...
for i in range(1, 16):
    ...
    print(f'image {os.path.join(output_folder, img_name)} created')
